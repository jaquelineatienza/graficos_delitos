export function delitos_barrios() {
  Promise.all([
    ...
    // Transformar datos
    let data = agrupado.map(([barrio, conteos]) => {
      const obj = { barrio };
    ...
    const barrios = data.map((d) => d.barrio);

    const width = 900;
    const height = barrios.length * 25 + 100;
    ...
    svg
      ...
      .data(data)
      .join("g")
      .attr("transform", (d) => `translate(0, ${y0(d.barrio)})`)
      .selectAll("rect")
